...
// Page<Course> coursePagePS =
// courseRepository.findAllByTeacherId(sessionUser.getTeacher().getId(),
// pageable);
...
// Teacher teacherPS =
// teacherRepository.findById(subjectPS.getTeacher().getId())
// .orElseThrow(() -> new Exception404("해당 선생님이 존재하지 않아요"));
// Paper paperPS = paperRepository.findBySubjectIdAndPaperVersion(subjectId,
// PaperVersion.ORIGINAL)
// .orElseThrow(() -> new Exception404("해당 교과목의 본평가 시험지를 찾을 수 없습니다."));
// List<Question> questionListPS =
// questionRepository.findAllByPaperId(paperPS.getId());
...
// return new DocumentModel.No2(paperPS.getEvaluationWay(), null,
// questionListPS);
// }
...
// List<Question> questionListPS =
// questionRepository.findAllByPaperId(paperPS.getId());
// List<SubjectElement> elementListPS =
// elementRepository.findAllBySubjectId(subjectId);
// Teacher teacherPS = teacherRepository.findById(1L)
// .orElseThrow(() -> new Exception404("해당 시험에 선생님이 존재하지 않아서 사인을 찾을 수 없어요"));
// return new DocumentModel.No3(paperPS, elementListPS, questionListPS,
// teacherPS);
// }

// public DocumentModel.No4 no4(Long subjectId, Integer currentIndex) {
// List<Exam> examListPS =
// examRepository.findBySubjectIdAndIsUseOrderByStudentNameAsc(subjectId);
...
// Integer nextIndex = currentIndex < examListPS.size() - 1 ? currentIndex + 1 :
// null;
...
// return new DocumentModel.No4(examPS, elementListPS, teacherPS, prevIndex,
// nextIndex, currentIndex);
// }

// public DocumentModel.No5 no5(Long subjectId) {
// List<Exam> examListPS =
// examRepository.findAllBySubjectIdAndPaperVersion(subjectId,
// PaperVersion.ORIGINAL);
// List<Exam> reExamListPS =
// examRepository.findAllBySubjectIdAndPaperVersion(subjectId,
// PaperVersion.RETEST);
// Teacher teacherPS =
// teacherRepository.findById(examListPS.get(0).getTeacher().getId())
// .orElseThrow(() -> new Exception404("해당 선생님이 존재하지 않아요"));
