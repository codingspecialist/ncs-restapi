...
public class DocumentController {
    ...
    public String no1(@PathVariable("subjectId") Long subjectId, Model model) {
        ...
        if (modelData.paper().getEvaluationWay() == EvaluationWay.MCQ) {
            var respDTO = new DocumentResponse.No1McqDTO(modelData.subject(), modelData.questions(),
                    modelData.teacher().getSign(), modelData.paper());
            // DB에 밀어넣기
            model.addAttribute("model", respDTO);
            return "document/no1-mcq";
        } else {
            var respDTO = new DocumentResponse.No1RubricDTO(modelData.subject(), modelData.questions(),
                    modelData.teacher().getSign(), modelData.paper());
            model.addAttribute("model", respDTO);
    ...
    public ResponseEntity<?> no2(@PathVariable("subjectId") Long subjectId, Model model)
    ...
    public String no3(@PathVariable("subjectId") Long subjectId, Model model) {
        ...
        if (modelData.paper().getEvaluationWay() == EvaluationWay.MCQ) {
            var respDTO = new DocumentResponse.No3McqDTO(modelData.paper(), modelData.elements(), modelData.questions(),
                    modelData.teacher());
            model.addAttribute("model", respDTO);
            return "document/no3-mcq";
        } else {
            var respDTO = new DocumentResponse.No3RubricDTO(modelData.paper(), modelData.questions(),
                    modelData.teacher());
            model.addAttribute("model", respDTO);
