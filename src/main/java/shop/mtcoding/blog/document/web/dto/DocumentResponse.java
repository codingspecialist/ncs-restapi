...
// private String sa; // 교과목ID로 paper를 조회하는데, isUse가 true걸로, 그 중에
// isReEvaluation이 true인것들의 count값
// private String ah; // 해당 사항 없음!!
// private String ja; // 교과목ID로 paper를 조회하는데, isUse가 true걸로, 그 중에
// isReEvaluation이 true인것들의 평가일자
// // 0번지하나만!!
...
// Paper rePaper =
// reExams.stream().map(Exam::getPaper).findFirst().orElse(null);
...
// + paper.getSubject().getCourse().getEndDate() + " (" +
// paper.getSubject().getCourse().getTotalTime()
// + "시간)";
// this.da = paper.getSubject().getTitle() + " (" +
...
// .filter(student -> student.getDropOutDate() == null
// || student.getDropOutDate().isAfter(paper.getEvaluationDate()))
// .count();
...
// this.ba = "재적 " + allStudentCount + "명 / 실시 " + doExamCount + "명" + temp1 +
// temp2;
...
// subject.getElements().stream().map(SubjectElement::getTitle).toList();
// this.questions = questions.stream().map(question -> new
// QuestionDTO(question)).toList();
// }
...
// public No1RubricDTO(Subject subject, List<Question> questions, String sign,
// Paper paper) {
...
// .toList();
// this.rubricSubmitFormats =
// MyUtil.parseMultilineWithoutHyphen(paper.getTaskSubmitFormat());

// // TODO (결시자, 재평가자야 0.9 프로 Subject에서 받기)
// this.scorePolicies = Arrays.asList("본평가 배점 : 평가점수 X 1.0", "재평가 배점 : 평가점수 X
// 0.9", "결시자 배점 : 평가점수 X 0.9");
...
// this.options =
// question.getQuestionOptions().stream().map(OptionDTO::new).toList();
// }
...
// // 교과목 조회, 교과목의 본평가 시험지로 출제된 문제 목록 조회
// public No1McqDTO(Subject subject, List<Question> questions, String sign,
// Paper paper) {
// this.courseTitle = subject.getCourse().getTitle();
// this.courseRound = subject.getCourse().getRound();
// this.courseStartDate = subject.getCourse().getStartDate().toString();
...
// // MyUtil.parseMultilineWithoutHyphen(paper.getSubmissionFormat());
// // TODO (결시자, 재평가자야 0.9 프로 Subject에서 받기)
// double scorePolicy = paper.getSubject().getScorePolicy();
// this.scorePolicies = Arrays.asList("본평가 배점 : 평가점수 X 1.0", "재평가 배점 : 평가점수 X "
// + scorePolicy,
// "결시자 배점 : 평가점수 X " + scorePolicy);

// this.subjectElements = subject.getElements().stream().map(subjectElement ->
...
// this.options =
// question.getQuestionOptions().stream().map(QuestionDTO.OptionDTO::new).toList();
// }
...
// public No3McqDTO(Paper paper, List<SubjectElement> subjectElements,
...
// this.evaluationDevice = paper.getEvaluationDevice();
// this.subjectTitle = paper.getSubject().getTitle();
// this.subjectElements = subjectElements.stream().map(se ->
// se.getTitle()).toList();
...
// public No4McqDTO(Exam exam, List<SubjectElement> subjectElements, Teacher
// teacher, Integer prevIndex,
...
// .map(option -> new AnswerDTO.OptionDTO(option, selectedOptionNo)).toList();
// }
...
// public No4RubricDTO(Exam exam, List<SubjectElement> subjectElements, Teacher
// teacher, Integer prevIndex,
// Integer nextIndex, Integer currentIndex) {
// this.subjectId = exam.getPaper().getSubject().getId();
// this.courseId = exam.getPaper().getSubject().getCourse().getId();
...
// this.codeReviewFeedbackPrLink =
// answer.getExamResult().getCodeReviewFeedbackPrLink();
// this.scenarios =
// MyUtil.parseMultilineWithoutHyphen(answer.getQuestion().getSummary());
// this.options = answer.getQuestion().getQuestionOptions().stream()
// .map(option -> new AnswerDTO.OptionDTO(option, selectedOptionNo)).toList();
// }
