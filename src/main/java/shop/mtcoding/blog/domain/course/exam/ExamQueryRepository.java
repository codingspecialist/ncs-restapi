...
public class ExamQueryRepository {
    ...
    public List<ExamModel.Result> findExamResult(Long subjectId) {
        ...
        for (Object[] row : rows) {
            ...
            Long studentId = row[9] != null ? ((Number) row[9]).longValue() : null;
            Long paperId = row[10] != null ? ((Number) row[10]).longValue() : null;
            Boolean isAbsent = false;

            resultList.add(new ExamModel.Result(examId,
                    studentName, subjectTitle, examState, teacherName,
                    score, grade, passState, reExamReason, studentId, paperId, isAbsent
            ));
