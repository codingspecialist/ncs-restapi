package shop.mtcoding.blog.domain.user.adapter.in.web;
...
import shop.mtcoding.blog.domain.user.adapter.in.web.dto.UserRequest;
import shop.mtcoding.blog.domain.user.adapter.in.web.dto.UserResponse;
...
public class UserController {
    ...
    public ResponseEntity<?> teacherJoin(@RequestBody UserRequest.TeacherJoin reqDTO) {
        var input = UserCommand.TeacherJoin.from(reqDTO);
        var output = teacherUseCase.강사회원가입(input);
    ...
    public ResponseEntity<?> studentJoin(@RequestBody UserRequest.StudentJoin reqDTO) {
        var input = UserCommand.StudentJoin.from(reqDTO);
        var output = studentUseCase.학생회원가입(input);
        var respDTO = UserResponse.Item.from(output);
        return ResponseEntity.ok(Resp.ok(respDTO));
    }
    ...
    public ResponseEntity<?> empJoin(@RequestBody UserRequest.EmpJoin reqDTO) {
        var input = UserCommand.EmpJoin.from(reqDTO);
        var output = empUseCase.직원회원가입(input);
        var respDTO = UserResponse.Item.from(output);
        return ResponseEntity.ok(Resp.ok(respDTO));
    }
    ...
    public ResponseEntity<?> login(@RequestBody UserRequest.Login reqDTO) {
        var input = UserCommand.Login.from(reqDTO);
        var output = userUseCase.로그인(input);
